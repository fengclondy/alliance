package com.ai.aso.common.db.page;
...
public class PageHelper implements Interceptor {
    ...
    private static boolean rowBoundsWithCount = true;
    ...
    private String getCountSql(String sql) {
		...
		int beginIndex=sql.toUpperCase().indexOf("FROM");
		int endIndex=sql.toUpperCase().indexOf("ORDER");
		if(endIndex>0)
            sql= sql.substring(0,endIndex-1);
		if(beginIndex>0)
			sql=sql.substring(beginIndex, sql.length());
