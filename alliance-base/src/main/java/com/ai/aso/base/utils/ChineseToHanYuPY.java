...
public class ChineseToHanYuPY {
	...
	private static final String DIC_PATH = "/config" + File.separator + "duoyinzi_dic.txt";
	 ...
	 * @return
	 */
	public static String getPinyinConvertJianPin(String pinyin) {
		String strChar = "";
		char arr[] = pinyin.toCharArray(); // 将字符串转化成char型数组
							...
							if (!flag) { // 都没有找到，匹配默认的 读音 大
								...
								for (int x = 0; x < len; x++) {
									...
									if (keyList != null && (keyList.contains(s))) {
										...
										// pinyin.append(results[x]);
										// //如果不需要拼音首字母大写 ，直接返回即可
										pinyin.append(convertInitialToUpperCase(py));// 拼音首字母
																						// 大写
										break;
									}
								}
	...
	public static void main(String[] args) {
		...
		System.out.println("简拼：" + getPinyinJianPin(str));
